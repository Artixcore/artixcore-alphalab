...
class ArtixcoreAlphaLabPredictor(Predictor):
    """Artixcore AlphaLab v0.23: balanced history-bridged raw/rank target ensemble."""
    ...
    def train(self, features, target):
        ...
        try:
            ...
            dtrain_raw = xgb.DMatrix(matrix, label=y_values, weight=weights)
            dtrain_rank = xgb.DMatrix(matrix, label=rank_values, weight=weights)
    ...
    def predict(self, features):
        ...
        try:
            ...
            tree_b = self.rank_scale_ * self.xgb_model_b_.predict(xgb.DMatrix(matrix)).astype(np.float32)
