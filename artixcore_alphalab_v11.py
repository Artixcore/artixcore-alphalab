...
class ArtixcoreAlphaLabPredictor(Predictor):
    """Artixcore AlphaLab v0.11, volatility-normalized robust residual ensemble."""
    ...
    _Z_WEIGHT = 0.12
    _RANK_WEIGHT = 0.12
    _RESIDUAL_WEIGHT = 0.11

    _HUBER_C = 1.50
    _TARGET_QUANTILE = 0.995
    _Z_CLIP = 2.50
    ...
    def __init__(self):
        ...
        self.n_xgb_rounds = 14

        self.raw_coef_ = None
        self.raw_intercept_ = 0.0
        self.z_coef_ = None
        self.z_intercept_ = 0.0
        self.z_scale_ = 1.0
    ...
    def _group_standardized_target(self, y):
        if isinstance(y.index, pd.MultiIndex):
            group_std = y.groupby(level=0).transform("std").astype(np.float32)
        else:
            std = float(y.std()) if len(y) else 1.0
            group_std = pd.Series(std, index=y.index, dtype=np.float32)
        group_std = group_std.replace([np.inf, -np.inf], np.nan).fillna(0.0)
        values = y.to_numpy(dtype=np.float32)
        scales = group_std.to_numpy(dtype=np.float32)
        scales[~np.isfinite(scales) | (scales < 1.0e-6)] = 1.0
        standardized = values / scales
        return np.clip(standardized, -self._Z_CLIP, self._Z_CLIP).astype(np.float32)
    ...
    def train(self, features, target):
        ...
        try:
            ...
            target_limit = (
                np.nanquantile(np.abs(target_values), self._TARGET_QUANTILE)
            ...
            z_values = self._group_standardized_target(sampled_y)
            self.z_intercept_ = float(z_values.mean()) if z_values.size else 0.0
            self.z_coef_ = self._ridge(
                matrix,
                z_values - self.z_intercept_,
                robust_weights,
                self._Z_ALPHA,
            )
            ...
            z_std = float(np.std(z_values)) if z_values.size else 1.0
            rank_std = float(np.std(rank_values)) if rank_values.size else 1.0
            self.z_scale_ = (
                target_std / z_std
                if target_std > 1.0e-8 and z_std > 1.0e-8
                else 1.0
            )
            self.rank_scale_ = (
                target_std / rank_std
            ...
            z_prediction = self.z_scale_ * (
                self.z_intercept_ + matrix @ self.z_coef_
            )
