...
class ArtixcoreAlphaLabPredictor(Predictor):
    """AlphaLab v0.37: v33 feature engine with a Ridge and rank-Ridge ablation."""
    ...
    def _rank(frame):
        ...
        denom = 0.5 * max(n - 1, 1)
        return (
            (frame.rank(axis=1, method="average") - 0.5 * (n + 1)) / denom
        ).astype(np.float32)

    @staticmethod
    def _block(name, frame):
        out = frame.replace([np.inf, -np.inf], np.nan).astype(
    ...
    def _levels(self, columns):
        ...
        if asset_level is None:
            remaining = [
                i for i in range(columns.nlevels) if i != feature_level
            ]
            asset_level = (
    ...
    def _long(self, panel, target=None, assets=None):
        x = self._stack(panel, "asset").replace(
            [np.inf, -np.inf], np.nan
        )
        if target is None:
            return x
    ...
    def _select(self, x):
        ...
        for i, name in enumerate(probe.columns):
            ...
            if (
                finite.mean() >= 0.05
                and np.nanstd(values[finite, i]) >= 1e-8
            ):
                usable.append(name)
        priority = {
            name: i for i, name in enumerate(self.FEATURE_PRIORITY)
        }
    ...
    def _sample_times(self, index):
        ...
        positions = np.flatnonzero(np.isin(codes, chosen)).astype(
            np.int64
        )
    ...
    def _fit_preprocessor(self, x):
        ...
        if missing.any():
            values[missing] = np.take(
                self.impute_, np.where(missing)[1]
            )
        self.low_ = np.nanquantile(values, 0.01, axis=0).astype(
            np.float32
        )
        self.high_ = np.nanquantile(values, 0.99, axis=0).astype(
            np.float32
        )
        bad = (
    ...
    def _weights(codes, total):
        ...
        weights = np.exp(
            ...
            / max(
                1.0,
                total * ArtixcoreAlphaLabPredictor.DECAY,
            )
        ).astype(np.float32)
    ...
    def _finish(self, prediction):
        ...
        return (
            prediction.sub(prediction.mean(axis=1), axis=0)
            .fillna(0.0)
            .astype(np.float32)
        )
    ...
    def _fallback(self, features, assets):
        try:
            ...
            name = next(
                (n for n in names if n.endswith("__rank")), names[0]
            )
            return self._finish(
                panel[name]
    ...
    def train(self, features, target):
        self.is_trained_, self.training_error_, self.fallback_used_ = (
            False,
            None,
            False,
        )
        ...
        try:
            ...
            limit = (
                float(np.nanquantile(np.abs(raw), 0.995))
                if raw.size
                else 1.0
            )
            ...
            self.prediction_clip_ = float(
                np.clip(3.0 * limit, 1e-6, 10.0)
            )
            if isinstance(ys.index, pd.MultiIndex):
                ranked = ys.groupby(level=0).rank(
            ...
            self.rank_intercept_ = float(
                np.average(rank, weights=weights)
            )
            self.rank_coef_ = self._ridge(
                matrix,
                rank - self.rank_intercept_,
                weights,
                self.RANK_ALPHA,
            )
            ...
            self.training_times_ = (
                len(
                    pd.Index(
                        xs.index.get_level_values(0)
                    ).unique()
                )
                if isinstance(xs.index, pd.MultiIndex)
                else len(xs)
            )
    ...
    def predict(self, features):
        ...
        try:
            ...
            series = pd.Series(
                np.nan_to_num(raw), index=x.index, dtype=np.float32
            )
