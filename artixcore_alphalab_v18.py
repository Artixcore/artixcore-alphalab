...
class ArtixcoreAlphaLabPredictor(Predictor):
    """Artixcore AlphaLab v0.18: compact, shape-safe v0.15 refinement."""
    ...
    def train(self, features, target):
        ...
        try:
            ...
            ages = (len(x) - 1) - positions.astype(np.float32)
            weights = np.exp(-ages / max(1.0, len(x) * self._DECAY)).astype(np.float32)
            ...
            self.raw_intercept_ = float(yv.mean())
            ...
            self.rank_scale_ = float(np.std(yv) / max(np.std(rv), 1e-8))
