...
class ArtixcoreAlphaLabPredictor(Predictor):
    """AlphaLab v0.35: compact causal multi-regime feature ensemble."""
    ...
    FEATURE_PRIORITY = (
        ...
        "Feature.1__mom3vol10", "Feature.1__mom5vol20",
        "Feature.1__rank_persist", "Feature.1__rank_turnover",
        "Feature.1__ewma_resid", "Feature.1__z10",
        "Feature.1__range10", "Feature.1__range20",
        "Feature.1__trend5_20", "Feature.1__vol_ratio",
        "Feature.2__reversal5", "Feature.3__ewma_resid",
        "interaction__rank12", "interaction__rank13",
        "interaction__core", "regime__dispersion",
    )
    ...
    def __init__(self):
        ...
        self.max_features = 34
        self.history_rows = 72
    ...
    @staticmethod
    def _broadcast_series(series, columns):
        values = np.repeat(
            series.to_numpy(dtype=np.float32)[:, None],
            len(columns),
            axis=1,
        )
    ...
    def _features(self, features):
        ...
        for index, name in enumerate(names):
            ...
            if index == 0:
                ...
                low10 = raw.rolling(10, min_periods=3).min()
                high10 = raw.rolling(10, min_periods=3).max()
                low20 = raw.rolling(20, min_periods=4).min()
                high20 = raw.rolling(20, min_periods=4).max()
                ...
                blocks.extend(
                    [
                        ...
                        self._block(
                            name + "__mom3vol10",
                            (raw - raw.shift(3)) / (vol10 + 1e-6),
                        ),
                        self._block(
                            name + "__mom5vol20",
                            (raw - raw.shift(5)) / (vol20 + 1e-6),
                        ),
                        self._block(
                            name + "__rank_persist",
                            rank * rank_lag1,
                        ...
                        self._block(
                            name + "__range10",
                            (raw - low10) / (high10 - low10 + 1e-6) - 0.5,
                        ),
                        self._block(
                            name + "__range20",
                            (raw - low20) / (high20 - low20 + 1e-6) - 0.5,
                        ),
                        self._block(
                            name + "__trend5_20", ma5 - ma20
                        ),
                        self._block(
                            name + "__vol_ratio",
                            vol10 / (vol20 + 1e-6),
                ...
                ma5 = raw.rolling(5, min_periods=2).mean()
                blocks.append(
                    self._block(name + "__reversal5", ma5 - raw)
                )
                ...
                blocks.append(
                    self._block(name + "__ewma_resid", raw - ewma8)
                )
        ...
        if len(names) >= 3:
            ...
            blocks.extend(
                [
                    self._block(
                        "interaction__rank13", rank1 - rank3
                    ),
                    self._block(
                        "interaction__core",
                        0.55 * rank1 - 0.30 * rank2 + 0.15 * rank3,
                    ),
                ]
            )

        dispersion = frames[names[0]].std(axis=1, ddof=0)
        regime_dispersion = self._broadcast_series(
            dispersion, frames[names[0]].columns
        )
        blocks.append(
            self._block("regime__dispersion", regime_dispersion)
        )
    ...
    def _sample_complete_times(self, index):
        ...
        typical_assets = max(1, int(np.median(sizes[sizes > 0])))
    ...
    def _fallback(self, features, assets):
        try:
            ...
            preferred = next(
                (name for name in names if name.endswith("__rank")),
                names[0],
            )
