...
class ArtixcoreAlphaLabPredictor(Predictor):
    """AlphaLab v0.34A: compact causal ensemble with fixed conviction shaping."""
    ...
    # Fixed portfolio-construction constants. These never use target data.
    SHAPE_POWER = 1.25
    RAW_SHAPE_BLEND = 0.20
    SOFT_ZONE = 0.25
    SOFT_ZONE_MULTIPLIER = 0.35
    ...
    def _rank(frame):
        ...
        denominator = 0.5 * max(n - 1, 1)
        return (
            (frame.rank(axis=1, method="average") - 0.5 * (n + 1))
            / denominator
        ).astype(np.float32)
    ...
    def _features(self, features):
        ...
        for index, name in enumerate(names):
            ...
            if index == 0:
                ...
                ewma5 = raw.ewm(
                    span=5, adjust=False, min_periods=2
                ).mean()
                blocks.extend(
                    [
    ...
    def _long(self, panel, target=None, assets=None):
        features = self._stack(panel, "asset").replace(
            [np.inf, -np.inf], np.nan
        )
        if target is None:
            return features
        response = self._stack(self._target(target, panel.index, assets))
        features, response = features.align(response, join="inner", axis=0)
        valid = response.replace([np.inf, -np.inf], np.nan).notna()
        return features.loc[valid], response.loc[valid].astype(np.float32)
    ...
    def _select(self, features):
        ...
        for index, name in enumerate(probe.columns):
            ...
            if finite.mean() < 0.05:
                continue
            if np.nanstd(values[finite, index]) < 1e-8:
                continue
            usable.append(name)
    ...
    def _transform(self, features):
        ...
        if missing.any():
            values[missing] = np.take(self.impute_, np.where(missing)[1])
        values = (
            np.clip(values, self.low_, self.high_) - self.center_
        ) / self.scale_
    ...
    def _weights(codes, total):
        ...
        denominator = max(
            1.0, total * ArtixcoreAlphaLabPredictor.DECAY
        )
    ...
    def _shape_predictions(self, prediction):
        """Apply one fixed cross-sectional conviction transformation."""
        centered = prediction.sub(prediction.mean(axis=1), axis=0).fillna(0.0)
        values = centered.to_numpy(np.float32, copy=True)
        if values.size == 0:
            return centered.astype(np.float32)

        asset_count = values.shape[1]
        ranks = pd.DataFrame(values).rank(
            axis=1, method="average", pct=True
        ).to_numpy(np.float32)
        rank_score = 2.0 * ranks - 1.0
        shaped = np.sign(rank_score) * np.power(
            np.abs(rank_score), self.SHAPE_POWER
        )

        weak = np.abs(rank_score) < self.SOFT_ZONE
        shaped[weak] *= self.SOFT_ZONE_MULTIPLIER

        row_scale = np.std(values, axis=1, keepdims=True)
        normalized_raw = np.divide(
            values,
            row_scale,
            out=np.zeros_like(values),
            where=row_scale > 1e-8,
        )
        normalized_raw = np.tanh(normalized_raw)

        combined = (
            (1.0 - self.RAW_SHAPE_BLEND) * shaped
            + self.RAW_SHAPE_BLEND * normalized_raw
        )
    ...
    def _finish(self, prediction):
        ...
        prediction = self._shape_predictions(prediction)
        return prediction.sub(
    ...
    def _fallback(self, features, assets):
        try:
            ...
            name = next(
                (value for value in names if value.endswith("__rank")),
                names[0],
            )
            prediction = panel[name].reindex(
    ...
    def train(self, features, target):
        ...
        try:
            ...
            positions, codes, total = self._sample_times(
                matrix_frame.index
            )
            ...
            raw_target = np.nan_to_num(
                sampled_target.to_numpy(np.float32)
            )
            target_limit = (
            ...
            rank_target = np.nan_to_num(
                ((ranked_target - 0.5) * 2.0).to_numpy(np.float32)
            )
            ...
            target_std = float(np.std(raw_target))
            ...
            self.tree_model_ = xgb.train(
                ...
                xgb.DMatrix(
                    matrix, label=raw_target, weight=weights
                ),
                num_boost_round=9,
            )
