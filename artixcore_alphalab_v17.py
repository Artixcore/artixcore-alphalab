...
class ArtixcoreAlphaLabPredictor(Predictor):
    """Artixcore AlphaLab v0.17: fast robust-feature upgrade of the v0.8 core."""
    ...
    _PRIORITY = (
        ...
        "Feature.1__cs_robust_z", "Feature.2__cs_robust_z",
        "Feature.1__rank_delta", "Feature.2__rank_delta",
        ...
        "Feature.1__roll_z", "Feature.1__mom_spread", "Feature.1__trend_accel",
        "Feature.1__cs_demean", "interaction__rank_spread", "interaction__rank_agree",
    )

    _XGB = {
        "objective": "reg:squarederror", "max_depth": 2, "eta": 0.045,
        "subsample": 0.82, "colsample_bytree": 0.82,
        "min_child_weight": 220, "reg_alpha": 0.03, "reg_lambda": 2.0,
    ...
    def __init__(self):
        try:
            ...
            pass
        self.max_train_rows = 72000
        self.max_features = 32
        self.n_xgb_rounds = 13
    ...
    @staticmethod
    def _robust_cs_z(frame):
        median = frame.median(axis=1)
        centered = frame.sub(median, axis=0)
        mad = centered.abs().median(axis=1).replace(0.0, np.nan)
        return centered.div(1.4826 * mad + 1e-6, axis=0).clip(-6.0, 6.0).astype(np.float32)
    ...
    def _features(self, features):
        ...
        if not frames:
            ...
            return empty
        names, blocks, ranks = list(frames), [], {}
        for i, name in enumerate(names):
            ...
            if i < 3:
                ...
                if i < 2:
                    blocks.append(self._block(f"{name}__cs_robust_z", self._robust_cs_z(raw)))
                    blocks.append(self._block(f"{name}__rank_delta", rank - rank.shift(1)))
            if i == 0:
                ...
                blocks.extend([
                    ...
                    self._block(f"{name}__trend_accel", ma5 - 2.0 * ma20 + ma60),
                    self._block(f"{name}__cs_demean", raw.sub(raw.median(axis=1), axis=0)),
                ])
        if len(names) >= 2:
            r1 = ranks.get(names[0], self._rank(frames[names[0]]))
            r2 = ranks.get(names[1], self._rank(frames[names[1]]))
            blocks.append(self._block("interaction__rank_spread", r1 - r2))
            blocks.append(self._block("interaction__rank_agree", 0.5 * (r1 + r2)))
    ...
    def _select(self, x):
        if x.empty:
            return []
        probe = x if len(x) <= 30000 else x.iloc[np.linspace(0, len(x) - 1, 30000, dtype=np.int64)]
    ...
    def train(self, features, target):
        ...
        try:
            ...
            limit = np.nanquantile(np.abs(yv), 0.997) if yv.size else 1.0
            if not np.isfinite(limit) or limit <= 0:
                limit = 1.0
            yv = np.clip(yv, -limit, limit)
    ...
    def predict(self, features):
        ...
        try:
            ...
            raw = self._RIDGE_WEIGHT * ridge + self._XGB_WEIGHT * tree + self._RANK_WEIGHT * rank
