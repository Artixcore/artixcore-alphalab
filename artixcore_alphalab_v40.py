...
class ArtixcoreAlphaLabPredictor(Predictor):
    """AlphaLab v0.40: v33 ensemble with training-only stable IC selection."""
    ...
    def __init__(self):
        ...
        self.max_selection_times = 256
        self.selection_redundancy = 0.985
        self.selection_decay = 0.45
        self.history_rows = 64
        self.selected_features_ = None
        self.feature_scores_ = {}
        self.selection_times_ = 0
        self.assets_ = []
        ...
        self.feature_time_ = self.fit_time_ = self.selection_time_ = 0.0
    ...
    def _selection_ic(self, x, y):
        if not isinstance(x.index, pd.MultiIndex):
            return None, None
        codes, times = pd.factorize(x.index.get_level_values(0), sort=False)
        total = len(times)
        if total < 8:
            return None, None
        if total <= self.max_selection_times:
            chosen = np.arange(total, dtype=np.int64)
        else:
            chosen = np.unique(np.linspace(0, total - 1, self.max_selection_times, dtype=np.int64))
        x_values = x.to_numpy(np.float32, copy=False)
        y_values = y.to_numpy(np.float32, copy=False)
        ics = np.full((len(chosen), x.shape[1]), np.nan, dtype=np.float32)
        for row, code in enumerate(chosen):
            positions = np.flatnonzero(codes == code)
            if positions.size < 4:
                continue
            block = x_values[positions]
            target = y_values[positions]
            target_valid = np.isfinite(target)
            if target_valid.sum() < 4:
                continue
            block = block[target_valid]
            target = target[target_valid]
            ranked_x = pd.DataFrame(block).rank(axis=0, method="average", pct=True).to_numpy(np.float32)
            ranked_y = pd.Series(target).rank(method="average", pct=True).to_numpy(np.float32)
            counts = np.isfinite(ranked_x).sum(axis=0)
            x_mean = np.divide(
                np.nansum(ranked_x, axis=0), counts,
                out=np.zeros(ranked_x.shape[1], dtype=np.float32),
                where=counts > 0,
            )
            y_mean = float(np.mean(ranked_y))
            x_centered = ranked_x - x_mean
            y_centered = ranked_y - y_mean
            numerator = np.nansum(x_centered * y_centered[:, None], axis=0)
            denominator = np.sqrt(
                np.nansum(x_centered * x_centered, axis=0)
                * float(np.sum(y_centered * y_centered))
            )
            valid = (counts >= 4) & np.isfinite(denominator) & (denominator > 1e-10)
            ics[row, valid] = (numerator[valid] / denominator[valid]).astype(np.float32)
        return ics, chosen
    ...
    def _select(self, x, y):
        ...
        scored_x = x.loc[:, usable]
        ics, chosen = self._selection_ic(scored_x, y)
        scores = np.zeros(len(usable), dtype=np.float64)
        if ics is not None and chosen is not None:
            total = int(chosen[-1]) + 1
            ages = (total - 1) - chosen.astype(np.float64)
            weights = np.exp(-ages / max(1.0, total * self.selection_decay))
            weights /= max(float(weights.mean()), 1e-12)
            mean_all = self._weighted_nanmean(ics, weights)
            centered = ics - mean_all[None, :]
            variance = self._weighted_nanmean(centered * centered, weights)
            dispersion = np.sqrt(np.maximum(variance, 0.0))
            recent_start = max(0, total - max(8, int(total * 0.35)))
            recent_mask = chosen >= recent_start
            if recent_mask.sum() < 3:
                recent_mask[-min(3, len(recent_mask)):] = True
            early_mask = ~recent_mask
            mean_recent = self._weighted_nanmean(ics[recent_mask], weights[recent_mask])
            if early_mask.sum() >= 3:
                mean_early = self._weighted_nanmean(ics[early_mask], weights[early_mask])
            else:
                mean_early = mean_all.copy()
            finite = np.isfinite(ics)
            counts = finite.sum(axis=0)
            coverage = counts / max(1, ics.shape[0])
            direction = np.sign(mean_all)
            same_direction = finite & (np.sign(ics) == direction[None, :])
            consistency = np.divide(
                same_direction.sum(axis=0), counts,
                out=np.zeros_like(mean_all), where=counts > 0,
            )
            agreement = np.where(
                np.isfinite(mean_early) & np.isfinite(mean_recent)
                & (mean_early * mean_recent >= 0.0),
                1.0, 0.35,
            )
            blended = 0.40 * mean_all + 0.60 * mean_recent
            scores = (
                np.abs(blended)
                * (0.50 + 0.50 * consistency)
                * agreement
                * np.sqrt(np.maximum(coverage, 0.0))
                / (0.08 + np.nan_to_num(dispersion, nan=1.0))
            )
            scores = np.nan_to_num(scores, nan=0.0, posinf=0.0, neginf=0.0)
            self.selection_times_ = int(len(chosen))

        self.feature_scores_ = {
            name: float(score) for name, score in zip(usable, scores)
        }
        order = sorted(
            range(len(usable)),
        ...
        probe = scored_x if len(scored_x) <= 40000 else scored_x.iloc[
            np.linspace(0, len(scored_x) - 1, 40000, dtype=np.int64)
        ]
        probe_values = probe.to_numpy(np.float64, copy=True)
        probe_values[~np.isfinite(probe_values)] = np.nan
        medians = np.nanmedian(probe_values, axis=0)
        medians[~np.isfinite(medians)] = 0.0
        missing = ~np.isfinite(probe_values)
        if missing.any():
            probe_values[missing] = np.take(medians, np.where(missing)[1])
        probe_values -= probe_values.mean(axis=0)
        scales = probe_values.std(axis=0)
        scales[~np.isfinite(scales) | (scales < 1e-8)] = 1.0
        probe_values /= scales
        correlation = (probe_values.T @ probe_values) / max(1, len(probe_values))
        correlation = np.clip(
            np.nan_to_num(correlation, nan=0.0, posinf=0.0, neginf=0.0),
            -1.0, 1.0,
        )

        name_to_index = {name: i for i, name in enumerate(usable)}
        selected = [name for name in self.CORE_FEATURES if name in name_to_index]
        selected_indices = [name_to_index[name] for name in selected]
        for i in order:
            name = usable[i]
            if name in selected:
                continue
            if selected_indices and np.max(np.abs(correlation[i, selected_indices])) >= self.selection_redundancy:
                continue
    ...
    def train(self, features, target):
        ...
        try:
            ...
            sampled_x, sampled_y = x.iloc[positions], y.iloc[positions]
            started = time.perf_counter()
            self.selected_features_ = self._select(sampled_x, sampled_y)
            self.selection_time_ = time.perf_counter() - started
